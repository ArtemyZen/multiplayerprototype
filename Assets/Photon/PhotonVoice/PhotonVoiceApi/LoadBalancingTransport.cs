...
namespace Photon.Voice
{
    ...
    /// </remarks>
    public class LoadBalancingTransport : Photon.Realtime.LoadBalancingClient, IVoiceTransport, IDisposable
    {
        ...
        {
            return this.OpChangeGroups(groupsToRemove, groupsToAdd);
        }
        ...
        RaiseEventArgs buildEvOptFromTargets(bool targetMe, int[] targetPlayers)
        {
            var opt = new RaiseEventArgs();
            if (targetMe)
            {
        ...
        class LBCLogger : ILogger
        {
            ...
            public Photon.Voice.LogLevel Level
            {
                get
                {
                    switch (lbt.LogLevel)
                    {
                        case Photon.Client.LogLevel.Debug:
                            return Photon.Voice.LogLevel.Debug;
                        case Photon.Client.LogLevel.Info:
                            return Photon.Voice.LogLevel.Info;
                        case Photon.Client.LogLevel.Warning:
                            return Photon.Voice.LogLevel.Warning;
                        case Photon.Client.LogLevel.Error:
            ...
            public void Log(Photon.Voice.LogLevel level, string fmt, params object[] args)
            {
                if (this.Level >= level)
                {
                    Photon.Client.LogLevel logLevel = Photon.Client.LogLevel.Debug;
                    if (level == Photon.Voice.LogLevel.Info) logLevel = Photon.Client.LogLevel.Info;
                    else if (level == Photon.Voice.LogLevel.Warning) logLevel = Photon.Client.LogLevel.Warning;
                    else if (level == Photon.Voice.LogLevel.Error) logLevel = Photon.Client.LogLevel.Error;
                    lbt.DebugReturn(logLevel, string.Format(fmt, args));
                }
            }
        }
